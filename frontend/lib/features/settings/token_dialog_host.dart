...
/// opens the [TokenDialog] as a modal when:
/// - the user has no token yet (first use), or
/// - the backend has flagged the current token as a 401.
///
/// The dialog is non-dismissable, so the user must enter a token to proceed.
/// Saving the token clears the unauthorized flag and closes the dialog; the
/// auth store then notifies listeners, which would re-evaluate this host
/// and find no reason to show the dialog again.
///
...
class _TokenDialogHostState extends ConsumerState<TokenDialogHost> {
  ...
  Widget build(BuildContext context) {
    // Re-evaluate whenever the auth store changes (token written, token
    // cleared, 401 flagged, 401 cleared).
  ...
  void _maybeShowDialog() {
    ...
    final needsToken = !authStore.hasToken || authStore.isUnauthorized;
    if (!needsToken || _isDialogOpen) return;

    _isDialogOpen = true;
    showDialog<void>(
      context: context,
      // Non-dismissable: the user must enter a token. The router already
      // forces every route to `/` while there is no valid token, so there
      // is nothing useful to dismiss to.
